...
RSpec.describe User, type: :model do
  ...
  describe 'Instance Method(s)' do
    ...
    describe '#best_customer_by_items' do
      it 'returns name and item_count of the user with the most item quantity from me' do
        merchant = create(:merchant)
        item = create(:item, user: merchant)
        ...
        il_user = create(:user, state: "IL")
        il_order = create(:shipped_order, user: il_user)
        il_order_item = create(:fulfilled_order_item, quantity: 100, item: item, order: il_order)

        expect(merchant.best_customer_by_items.name).to eq(il_user.name)
        expect(merchant.best_customer_by_items.items_ordered).to eq(100)
      end
    end
